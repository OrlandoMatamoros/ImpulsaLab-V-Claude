...
export default function VerificationPage() {
  ...
  const password = searchParams.get('p') || '' // Encriptado en base64
  ...
  useEffect(() => {
    // Enviar códigos al cargar la página
    if (email && !sessionStorage.getItem('codes_sent')) {
  ...
  const handleVerify = async () => {
    ...
    try {
      // Para verificación simplificada, usamos el mismo código para ambos
      const verifyResponse = await fetch('/api/verification/verify-codes', {
        ...
        body: JSON.stringify({
          ...
          smsCode: emailCodeStr // Usar el mismo código temporalmente
        })
      })
      ...
      const signupResponse = await fetch('/api/auth/create-verified-user', {
        ...
        body: JSON.stringify({
          ...
          consultantCode,
          emailVerified: true,
          phoneVerified: true // Marcamos como verificado aunque solo usemos email
        })
      })
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-4 p-3 bg-gray-100 rounded-lg">
            <p className="text-xs text-gray-600 text-center">
              🔧 Modo desarrollo: El código aparece en la consola del navegador (F12)
            </p>
          </div>
        )}
      </div>
